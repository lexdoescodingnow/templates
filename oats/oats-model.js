...
const OATS_CSS_URL = 'https://cdn.jsdelivr.net/gh/lexdoescodingnow/templates@1de2df3cac903652be8e4cc7bd18480bf66440b9/oats/oats-harvest-v1.css';
...
function oatsMarkup(d,s) {
  ...
  const lines=[`<div class="bh-oats oat-${d.type} oat-${d.slug}"${flow}>`,`<a class="oat-name" href="${oatsEscape(s.url)}">${oatsEscape(s.name)}</a>`,`<div class="oat-title">${oatsEscape(s.title)}</div>`];
  ...
  lines.push('<div class="oat-copy">',s.body,'</div>',d.type==='comms'?'<div class="oat-hardware" aria-hidden="true"></div>':'<div class="oat-mark" aria-hidden="true"></div>','</div>');
  return lines.join('\n');
}
