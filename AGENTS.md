...
- Include `[url]`, `[name]` and `[text]` placeholders, supplied placeholder GIFs and lorem ipsum where appropriate. GIF URLs supplied by the user are examples, never a fixed count requirement. Choose zero, one, two or more images according to each design; preserve convincing paired-image layouts, use single contact portraits where appropriate, and include image-free writing-led choices. Removing images must reclaim their space, and previews, editor defaults and posting snippets must agree. Do not put comments, hidden tips, instruction blocks or editing notes inside template HTML or CSS. Documentation belongs outside the template.
