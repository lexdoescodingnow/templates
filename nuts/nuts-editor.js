const nutsStates = new Map(NUTS_DESIGNS.map(d=>[d.slug,nutsDefaults(d)]));
let nutsCurrent = NUTS_DESIGNS[0];
const el=id=>document.getElementById(id);
function nutsFormat(value) { return value.replace(/\[(\/?)(b|i|u)\]/gi,(_,slash,tag)=>`<${slash}${tag.toLowerCase()}>`); }
function nutsBody(value) {
  const parsed=new DOMParser().parseFromString(nutsFormat(value),'text/html');
...
function nutsRead() {
  ...
  s.body=nutsBody(el('writing').value);
  return s;
}
function nutsDraw() {
  const s=nutsStates.get(nutsCurrent.slug);
  ...
  el('word-count').textContent=`${count} words${nutsCurrent.type==='bud'?' · buds are for replies of 100 words or fewer':''}`;
  el('word-count').classList.toggle('over',nutsCurrent.type==='bud'&&count>100);
}
function nutsGifFields() {
  const s=nutsStates.get(nutsCurrent.slug);
  ...
  s.gifs.forEach((gif,index)=>{
    ...
    input.addEventListener('input',()=>{const value=input.value.trim();gif.url=/^https?:\/\//i.test(value)?value:'';nutsDraw();});
    ...
    crop.addEventListener('input',()=>{gif.position=`50% ${crop.value}%`;nutsDraw();});label.append(crop);
    const remove=document.createElement('button');remove.type='button';remove.textContent='Remove';remove.addEventListener('click',()=>{s.gifs.splice(index,1);nutsGifFields();nutsDraw();});
    bottom.append(label,remove);row.append(input,bottom);el('gif-fields').append(row);
  });
}
function nutsSelect(slug) {
  nutsCurrent=NUTS_DESIGNS.find(d=>d.slug===slug)||NUTS_DESIGNS[0];
  const s=nutsStates.get(nutsCurrent.slug);
  for(const k of ['name','url','title','time','flow']) el(k).value=s[k];
  el('writing').value=s.body;
  el('comms-fields').hidden=nutsCurrent.type!=='comms';
  el('design-title').textContent=nutsCurrent.name;
  el('design-description').textContent=nutsCurrent.description;
  el('design-kind').textContent=`${nutsCurrent.type==='bud'?'Bud':nutsCurrent.type==='thread'?'Thread':'Comms'} ${nutsCurrent.number}`;
  for(const b of document.querySelectorAll('[data-design]'))b.setAttribute('aria-pressed',String(b.dataset.design===nutsCurrent.slug));
  el('raw-link').href=`https://github.com/lexdoescodingnow/templates/blob/main/nuts/${nutsFilename(nutsCurrent)}`;
  el('status').textContent='';nutsGifFields();nutsDraw();
}
for(const b of document.querySelectorAll('[data-design]'))b.addEventListener('click',()=>nutsSelect(b.dataset.design));
for(const k of ['name','url','title','time','flow','writing'])el(k).addEventListener('input',()=>{nutsRead();nutsDraw();});
el('add-gif').addEventListener('click',()=>{const s=nutsStates.get(nutsCurrent.slug);s.gifs.push({url:NUTS_GIFS[s.gifs.length%2],position:'50% 45%'});nutsGifFields();nutsDraw();});
el('reset').addEventListener('click',()=>{nutsStates.set(nutsCurrent.slug,nutsDefaults(nutsCurrent));nutsSelect(nutsCurrent.slug);});
...
el('download').addEventListener('click',()=>{const url=URL.createObjectURL(new Blob([el('code').value],{type:'text/plain;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download=nutsFilename(nutsCurrent);a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);el('status').textContent='Your edited snippet is ready.';});
...
nutsSelect(NUTS_DESIGNS[0].slug);
