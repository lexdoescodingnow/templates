const sesameStates = new Map(SESAME_DESIGNS.map(d=>[d.slug,sesameDefaults(d)]));
let sesameCurrent = SESAME_DESIGNS[0];
const el=id=>document.getElementById(id);
function sesameFormat(value) { return value.replace(/\[(\/?)(b|i|u)\]/gi,(_,slash,tag)=>`<${slash}${tag.toLowerCase()}>`); }
function sesameBody(value) {
  const parsed=new DOMParser().parseFromString(sesameFormat(value),'text/html');
...
function sesameRead() {
  ...
  s.body=sesameBody(el('writing').value);
  return s;
}
function sesameDraw() {
  const s=sesameStates.get(sesameCurrent.slug);
  ...
  el('word-count').textContent=`${count} words${sesameCurrent.type==='bud'?' · buds are for replies of 100 words or fewer':''}`;
  el('word-count').classList.toggle('over',sesameCurrent.type==='bud'&&count>100);
}
function sesameGifFields() {
  const s=sesameStates.get(sesameCurrent.slug);
  ...
  s.gifs.forEach((gif,index)=>{
    ...
    input.addEventListener('input',()=>{const value=input.value.trim();gif.url=/^https?:\/\//i.test(value)?value:'';sesameDraw();});
    ...
    crop.addEventListener('input',()=>{gif.position=`50% ${crop.value}%`;sesameDraw();});label.append(crop);
    const remove=document.createElement('button');remove.type='button';remove.textContent='Remove';remove.addEventListener('click',()=>{s.gifs.splice(index,1);sesameGifFields();sesameDraw();});
    bottom.append(label,remove);row.append(input,bottom);el('gif-fields').append(row);
  });
}
function sesameSelect(slug) {
  sesameCurrent=SESAME_DESIGNS.find(d=>d.slug===slug)||SESAME_DESIGNS[0];
  const s=sesameStates.get(sesameCurrent.slug);
  for(const k of ['name','url','title','time','flow']) el(k).value=s[k];
  el('writing').value=s.body;
  el('comms-fields').hidden=sesameCurrent.type!=='comms';
  el('design-title').textContent=sesameCurrent.name;
  el('design-description').textContent=sesameCurrent.description;
  el('design-kind').textContent=`${sesameCurrent.type==='bud'?'Bud':sesameCurrent.type==='thread'?'Thread':'Comms'} ${sesameCurrent.number}`;
  for(const b of document.querySelectorAll('[data-design]'))b.setAttribute('aria-pressed',String(b.dataset.design===sesameCurrent.slug));
  el('raw-link').href=`https://github.com/lexdoescodingnow/templates/blob/main/sesame/${sesameFilename(sesameCurrent)}`;
  el('status').textContent='';sesameGifFields();sesameDraw();
}
for(const b of document.querySelectorAll('[data-design]'))b.addEventListener('click',()=>sesameSelect(b.dataset.design));
for(const k of ['name','url','title','time','flow','writing'])el(k).addEventListener('input',()=>{sesameRead();sesameDraw();});
el('add-gif').addEventListener('click',()=>{const s=sesameStates.get(sesameCurrent.slug);s.gifs.push({url:SESAME_GIFS[s.gifs.length%2],position:'50% 45%'});sesameGifFields();sesameDraw();});
el('reset').addEventListener('click',()=>{sesameStates.set(sesameCurrent.slug,sesameDefaults(sesameCurrent));sesameSelect(sesameCurrent.slug);});
...
el('download').addEventListener('click',()=>{const url=URL.createObjectURL(new Blob([el('code').value],{type:'text/plain;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download=sesameFilename(sesameCurrent);a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);el('status').textContent='Your edited snippet is ready.';});
...
sesameSelect(SESAME_DESIGNS[0].slug);
