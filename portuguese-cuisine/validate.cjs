...
const model=require('./portuguese-cuisine-model.js');
const read=f=>fs.readFileSync(path.join(root,f),'utf8');
let checks=0;
function ok(value,message){assert.ok(value,message);checks++;}
const css=read('portuguese-cuisine-mesa-v1.css');
const cssErrors=[];
...
csstree.walk(ast,{visit:'Rule',enter(node){if(node.prelude?.type==='SelectorList')for(const selector of node.prelude.children){ok(csstree.generate(selector).includes('.bh-portuguese'),'All template selectors are scoped');}}});
ok(!/\/\*|<!--/.test(css),'No CSS comments');
ok(css.includes('html[color-mode="dark"] .bh-portuguese'),'Explicit dark mode');
ok(css.includes('html:not([color-mode="light"]):not([color-mode="dark"])'),'Explicit forum mode takes priority over system fallback');
ok(css.includes('--pt-forward:linear-gradient(110deg,var(--pt-ta),var(--pt-tb),var(--pt-tc))'),'Forward emphasis gradient');
ok(css.includes('--pt-reverse:linear-gradient(110deg,var(--pt-tc),var(--pt-tb),var(--pt-ta))'),'Reverse italic gradient');
...
for(const d of designs){
 const s=model.portugueseDefaults(d),code=read(model.portugueseFilename(d));
 ...
 for(const img of doc.querySelectorAll('img'))ok(model.PORTUGUESE_GIFS.includes(img.src),d.name+' exact supplied URL');
 ok(doc.querySelector('.ptg-copy').textContent.includes('Lorem ipsum'),d.name+' sample writing');
 if(d.type==='bud')ok(doc.querySelector('.ptg-copy').textContent.trim().split(/\s+/).length<=100,d.name+' under 100 words');
 if(d.type==='comms'){
  const short=new JSDOM(model.portugueseMarkup(d,{...s,body:'<p>First<p>Second<p>Third'})).window.document;
  ok(short.querySelectorAll('.ptg-copy>p').length===3,d.name+' successive opening paragraphs');
 }
 const absent=new JSDOM(model.portugueseMarkup(d,{...s,gifs:[]})).window.document;
 ok(!absent.querySelector('.ptg-media'),d.name+' removed media has no empty container');
 const many=new JSDOM(model.portugueseMarkup(d,{...s,gifs:Array(4).fill({url:model.PORTUGUESE_GIFS[0],position:'50% 35%'})})).window.document;
...
const dom=new JSDOM(read('portuguese-cuisine-collection-preview.html'),{runScripts:'dangerously',virtualConsole:vc,url:'https://example.invalid/preview'});
...
for(let i=0;i<15;i++){
 ...
 ok(doc.querySelector('#pv-code').value===read(model.portugueseFilename(designs[i])),designs[i].name+' editor defaults equal posting file');
}
doc.querySelectorAll('.pv-choice')[0].click();
input('pv-name','Seojun & Wenjun');input('pv-title','Dinner <for> two');input('pv-url','https://example.com/thread');
...
ok(code.includes('Dinner &lt;for&gt; two')&&code.includes('Seojun &amp; Wenjun'),'Editable fields escape HTML');
...
ok(doc.querySelector('.ptg-copy').textContent.length>1800,'Long writing retained');
doc.querySelectorAll('.pv-choice')[1].click();doc.querySelectorAll('.pv-choice')[0].click();
ok(doc.querySelector('#pv-name').value==='Seojun & Wenjun','Edits persist per design');
while(doc.querySelector('[data-remove]'))doc.querySelector('[data-remove]').click();
ok(!doc.querySelector('#pv-stage .ptg-media'),'Removing all images updates preview');
...
ok(doc.querySelector('#pv-code').value===read(model.portugueseFilename(designs[0])),'Reset restores defaults');
doc.querySelector('#pv-view-all').click();
ok(doc.querySelectorAll('#pv-gallery .bh-portuguese').length===15,'Gallery includes fifteen templates');
doc.querySelector('[data-open="8"]').click();
ok(doc.querySelector('#pv-design-title').textContent==='Doce Recado','Gallery edit opens selected design');
